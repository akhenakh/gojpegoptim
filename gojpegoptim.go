...
func EncodeOptimized(r io.Reader, quality int) (imgBytes []byte, err error) {
	csrcimg := (*C.uchar)(unsafe.Pointer(&imgBytes[0]))
	cinputsize := C.ulong(len(imgBytes))
	var coutimg **C.uchar
	var coutsize *C.ulong
	C.optimizeJPEG(csrcimg, cinputsize, coutimg, coutsize, C.int(quality))

	imgBytes = C.GoBytes(unsafe.Pointer(coutimg), C.int(*coutsize))
	return
}
